...
const ReplayDiagnostics=({run})=>{
  ...
  return <><section className="panel"><div className="panel-title">Replay Diagnostics</div><Metrics items={[
    ...
    ['Entries triggered',diagnostics.entries_triggered],['Trades completed',diagnostics.trades_completed],
  ]}/>{reasons.length>0&&<div className="table-wrap"><table><thead><tr><th>Top rejection reason</th><th>Count</th></tr></thead><tbody>{reasons.map(([reason,count])=><tr key={reason}><td>{reason.replaceAll('_',' ')}</td><td>{count}</td></tr>)}</tbody></table></div>}</section></>
}
...
export function Backtests(){
  ...
  return <div><div className="page-head"><div><p className="eyebrow">DETERMINISTIC REPLAY</p><h1>Backtests</h1><p className="subhead">Closed-candle, no-look-ahead simulation with explicit costs.</p></div></div>
    ...
    {selected&&<><Metrics items={[['Net PnL',money(selected.net_profit)],['Win Rate',`${Number(selected.win_rate).toFixed(1)}%`],['Profit Factor',selected.profit_factor?Number(selected.profit_factor).toFixed(2):'—'],['Expectancy',Number(selected.expectancy).toFixed(2)+' R'],['Average R',Number(selected.average_rr).toFixed(2)],['Max Drawdown',Number(selected.max_drawdown_pct).toFixed(2)+'%'],['Total Trades',selected.trades_taken]]}/>{selected.trades_taken===0&&<div className="alert">No trades were executed during this replay. The diagnostics below show why.</div>}<ReplayDiagnostics run={selected}/><section className="panel"><div className="panel-title">Equity / drawdown curve</div><div className="equity-bars">{(selected.settings_json?.equity_curve||[]).map(x=><i key={x.index} style={{height:`${20+Math.max(0,80-x.drawdown_pct*4)}%`}} title={`${money(x.equity)} / DD ${x.drawdown_pct.toFixed(2)}%`}/>)}</div></section><TradeTable rows={trades}/></>}
