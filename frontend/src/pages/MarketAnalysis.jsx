...
import { deriveHeikinAshiCandles } from '../chartRendering'
...
const cleanView={raw:false,majorSwings:true,internalSwings:false,bos:true,choch:true,fvg:true,liquidity:false,orderBlocks:true,sweeps:true,setups:true,entryZones:true,targets:true,primaryWave:true,alternateWaves:false,fibonacci:false,waveTargets:true}
const rawView={...Object.fromEntries(Object.keys(cleanView).map(key=>[key,false])),raw:true}

export default function MarketAnalysis(){
  const [symbol,setSymbol]=useState('BTCUSDT'),[timeframe,setTimeframe]=useState('1h')
  const [data,setData]=useState({candles:[],swings:[],structure:[],fvg:[],liquidity:[],orderBlocks:[],sweeps:[],setups:[],waveCounts:[],activePositions:[],analysis:null})
  const [chartSettings,setChartSettings]=useState(cleanView)
  const [candleMode,setCandleMode]=useState('real')
  const [wave3haTrades,setWave3haTrades]=useState([])
  ...
  useEffect(()=>{api.get('/paper-forward/elliott-wave3-heikin-ashi/trades',{params:{symbol}}).then(r=>setWave3haTrades(r.data)).catch(()=>setWave3haTrades([]))},[symbol])
  useEffect(()=>{
  ...
  const emaByIndex=useMemo(()=>data.candles.map((c,index,all)=>{const closes=all.slice(0,index+1).map(x=>+x.close),ema=period=>closes.length<period?null:closes.reduce((previous,current,i)=>i?current*(2/(period+1))+previous*(1-2/(period+1)):current,closes[0]);return {ema20:ema(20)?{time:Math.floor(new Date(c.open_time).getTime()/1000),value:ema(20)}:null,ema50:ema(50)?{time:Math.floor(new Date(c.open_time).getTime()/1000),value:ema(50)}:null,ema200:ema(200)?{time:Math.floor(new Date(c.open_time).getTime()/1000),value:ema(200)}:null}}),[data.candles])
  // Heikin Ashi is a visual-only candle-body swap for signal inspection - EMAs
  // always stay computed off real closes, and every overlay (setups, active
  // positions, wave points) keeps referencing real executable prices.
  const haCandles=useMemo(()=>deriveHeikinAshiCandles(data.candles),[data.candles])
  const chartCandles=useMemo(()=>(candleMode==='ha'?haCandles:data.candles).map((c,index)=>({...c,indicators:emaByIndex[index]})),[data.candles,haCandles,candleMode,emaByIndex])
  ...
  return <div>
    ...
    {chartSettings.raw&&<div className="notice good"><strong>RAW CANDLES ONLY · BINANCE · {symbol} · {timeframe}</strong></div>}
    {candleMode==='ha'&&<div className="notice good"><strong>HEIKIN ASHI · VISUAL REFERENCE ONLY</strong> — entries, stops, exits and analysis always use real OHLC, never these synthetic values.</div>}
    ...
    <section className="panel chart-panel"><div className="panel-title"><span>BINANCE · {symbol} · {timeframe}</span><div>{[['1D',1],['7D',7],['30D',30],['90D',90],['180D',180],['1Y',365]].map(([label,days])=><button key={label} onClick={()=>selectHistory(days)}>{label}</button>)}<button className="clean-button" onClick={()=>setChartSettings(rawView)}>Raw Candles</button><button className="clean-button" onClick={()=>setChartSettings(cleanView)}>Clean View</button><button className={`clean-button ${candleMode==='ha'?'active':''}`} onClick={()=>setCandleMode(m=>m==='ha'?'real':'ha')}>{candleMode==='ha'?'Show Real Candles':'Show Heikin Ashi'}</button><small>{loading?'Refreshing…':olderLoading?'Loading older history…':`${data.candles.length} candles`}</small></div></div><div className="chart-settings">{Object.entries(settingLabels).map(([key,label])=><label key={key}><input type="checkbox" checked={chartSettings[key]} onChange={e=>setChartSettings({...chartSettings,[key]:e.target.checked})}/>{label}</label>)}</div><MarketChart chartKey={`${symbol}:${timeframe}:${candleMode}`} candles={chartCandles} swings={data.swings} structure={data.structure} fvg={data.fvg} liquidity={data.liquidity} orderBlocks={data.orderBlocks} premiumDiscount={data.premiumDiscount} sweeps={data.sweeps} setups={data.setups} activePositions={data.activePositions} waveCounts={data.waveCounts} wave3haTrades={wave3haTrades} indicators={indicators} settings={chartSettings} onLoadOlder={loadOlder}/><div className="legend"><span className="ema20">EMA 20</span><span className="ema50">EMA 50</span><span>Elliott Primary / Alternates</span><span>Sweeps</span><span>Analysis setups · Entry / SL / TP</span><span>Active positions · Actual entry / originating geometry</span></div></section>
  </div>
}
