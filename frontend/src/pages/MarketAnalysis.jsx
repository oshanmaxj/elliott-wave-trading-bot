...
const settingLabels = { majorSwings: 'Major Swings', internalSwings: 'Internal Swings', bos: 'BOS', choch: 'CHoCH', fvg: 'FVG', liquidity: 'Liquidity', orderBlocks: 'Order Blocks' }

export default function MarketAnalysis() {
  const [symbol, setSymbol] = useState('BTCUSDT'), [timeframe, setTimeframe] = useState('1h')
  const [data, setData] = useState({ candles: [], swings: [], structure: [], fvg: [], liquidity: [], orderBlocks: [], analysis: null })
  const [chartSettings, setChartSettings] = useState({ majorSwings: true, internalSwings: true, bos: true, choch: true, fvg: true, liquidity: true, orderBlocks: true })
  ...
  useEffect(() => {
    ...
    socket.onmessage = event => { const message = JSON.parse(event.data); const payload = message.data || {}; if (payload.symbol === symbol && payload.timeframe === timeframe && ['candle_closed','swing_point','bos','choch','fvg_new','fvg_mitigation','liquidity_new','liquidity_sweep','order_block_new','order_block_mitigation','analysis_snapshot'].includes(message.type)) load() }
