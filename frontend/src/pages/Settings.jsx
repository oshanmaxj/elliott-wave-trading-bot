...
export default function Settings() {
  ...
  return <>
    ...
    <form className="panel settings" onSubmit={submit}>
      ...
      <h3>Wave-3 Heikin Ashi research</h3><div className="form-grid"><NumberField label="HA pullback candles" name="ha_pullback_min_candles" form={form} set={set} step={1}/><NumberField label="HA wick / body max" name="ha_wick_body_max_ratio" form={form} set={set}/><NumberField label="HA body / ATR min" name="ha_body_atr_min_ratio" form={form} set={set}/><NumberField label="Research score threshold" name="wave3_ha_research_score_threshold" form={form} set={set} step={1} max={100}/><NumberField label="Maximum stop / ATR" name="wave3_ha_max_stop_atr_ratio" form={form} set={set}/></div><Toggle label="Require next closed HA confirmation" name="ha_confirmation_required" form={form} set={set}/><p className="subhead">Research/paper only. Live automatic execution is locked off for this strategy.</p>
