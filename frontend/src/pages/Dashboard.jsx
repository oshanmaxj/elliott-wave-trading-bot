...
export default function Dashboard({ navigate }) {
  ...
  return <>
    ...
    <section className={`panel safety ${diagnostics?.closed_candles_processed === 0 ? 'bearish' : ''}`}><p className="eyebrow">STRATEGY PIPELINE · LAST 24 HOURS</p>{diagnostics ? <><div className="smc-cards">{[['Candles processed', diagnostics.closed_candles_processed], ['Strategy evaluations', diagnostics.strategy_evaluations], ['Candidates', diagnostics.candidates_generated], ['Rejected', diagnostics.candidates_rejected], ['Eligible', diagnostics.execution_eligible], ['Orders submitted', diagnostics.orders_submitted]].map(([label, value]) => <Card key={label} icon={Activity} label={label} value={value}/>)}</div><div className="smc-cards">{[['Swing points', 'swing_points_created'], ['Structure events', 'structure_events_created'], ['FVGs', 'fvg_zones_created'], ['Order blocks', 'order_blocks_created'], ['Liquidity sweeps', 'liquidity_sweeps_created'], ['Trade setups', 'trade_setups_created']].map(([label, key]) => <Card key={key} icon={Layers3} label={label} value={diagnostics.analysis_activity?.[key] ?? 0}/>)}</div>{diagnostics.closed_candles_processed === 0 && <p>Pipeline problem: the bot has not processed any closed candles in this period.</p>}<p>Main rejection reasons: {Object.entries(diagnostics.rejection_reasons || {}).sort((a,b) => b[1]-a[1]).slice(0, 5).map(([reason, count]) => `${reason.replaceAll('_', ' ')} (${count})`).join(' · ') || 'None'}</p></> : <p>Diagnostics unavailable.</p>}</section>
