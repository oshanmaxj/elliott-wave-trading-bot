...
class AnalysisBackfillStatus:
    ...
    def start(self, symbol: str, timeframe: str, total: int, started_at: datetime) -> None:
        self.running = True
        self.symbol = symbol
    ...
    def report(self) -> dict[str, Any]:
        ...
        progress = 0.0 if self.total_candles == 0 and self.running else (100.0 if self.total_candles == 0 else round(completed / self.total_candles * 100, 2))
        return {
...
class AnalysisBackfillService:
    def __init__(self, session_factory=SessionLocal, processor: Callable[..., Any] = process_closed_candle):
        self.session_factory = session_factory
        self.processor = processor

    def _candle_ids(self, symbol: str, timeframe: str, start_time: datetime | None, end_time: datetime | None, limit: int | None) -> tuple[int | None, list[int]]:
    ...
    async def run(self, symbol: str, timeframe: str, start_time: datetime | None = None, end_time: datetime | None = None, limit: int | None = None, rebuild: bool = False) -> dict[str, Any]:
        async with backfill_status.lock:
            started_at = _utcnow()
            started_perf = time.perf_counter()
            symbol_id, candle_ids = self._candle_ids(symbol, timeframe, start_time, end_time, limit)
            ...
            try:
                ...
                for candle_id in candle_ids:
                    if not rebuild:
                        ...
                        if existing_snapshot is not None and existing_snapshot.indicator_values_json.get("_smc_version") == 2:
                            ...
                            continue
                    try:
                        result = await self.processor(candle_id, broadcast=False, session_factory=self.session_factory)
                        ...
                        log_event("ERROR", "analysis_backfill", "candle_failed", str(exc), {"symbol": symbol, "timeframe": timeframe, "candle_id": candle_id})
                ...
                log_event("INFO", "analysis_backfill", "backfill_complete", "Historical analysis backfill completed", report)
                await broadcaster.broadcast("analysis_backfill_completed", jsonable_encoder(report))
                return report
            finally:
