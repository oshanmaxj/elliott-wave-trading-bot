...
class AutomaticTestnetExecutor:
    ...
    def __init__(self, session_factory=SessionLocal, client_factory=BinanceSpotClient, settings=None):
        self.session_factory = session_factory
        self.client_factory = client_factory
        self.settings = settings or get_settings()

    def _event(self, db, event_type, setup, symbol, *, order=None, reason=None, severity="INFO", metadata=None):
        details = {"symbol": symbol.symbol, "timeframe": setup.setup_timeframe, "strategy": setup.strategy, "side": "BUY" if setup.direction == "bullish" else "SELL", "reason": reason, "client_order_id": order.client_order_id if order else None, **(metadata or {})}
        db.add(ExecutionEvent(severity=severity, event_type=event_type, exchange="binance", environment="testnet", symbol_id=symbol.id, trade_setup_id=setup.id, execution_order_id=order.id if order else None, message=reason or event_type.replace("_", " "), metadata_json=details))
        log_event(severity, "execution", event_type, reason or event_type.replace("_", " "), {"setup_id": setup.id, "execution_order_id": order.id if order else None, **details})

    def _preflight_reasons(self, db, setup, symbol, manual_approved=False):
        runtime = db.scalar(select(BotRuntimeState).limit(1))
        reasons = []
        if self.settings.binance_environment != "testnet" or not self.settings.binance_execution_enabled:
    ...
    async def handoff(self, setup_id: int, *, manual_approved=False) -> dict:
        ...
        with self.session_factory() as db:
            ...
            existing = db.scalar(select(ExecutionOrder).where(ExecutionOrder.trade_setup_id == setup.id))
            if existing:
                return {"started": False, "reason": "duplicate_setup_window", "order_id": existing.id}
            reasons = self._preflight_reasons(db, setup, symbol, manual_approved)
            if reasons:
                self._event(db, "auto_execution_skipped", setup, symbol, reason=reasons[0], metadata={"reasons": reasons})
                db.commit()
        ...
        try:
            ...
            with self.session_factory() as db:
                ...
                if existing:
                    return {"started": False, "reason": "duplicate_setup_window", "order_id": existing.id}
                reasons = self._preflight_reasons(db, setup, symbol, manual_approved)
                decision = ExecutionRiskEngine(execution_settings).evaluate(db, setup, symbol, account, price, info, kill_switch=bool("kill_switch_enabled" in reasons))
                ...
                order = ExecutionOrder(environment="testnet", symbol_id=symbol.id, trade_setup_id=setup.id, client_order_id=cid, setup_fingerprint=setup_fingerprint(symbol.symbol, setup), side="BUY", order_type="MARKET", requested_quantity=decision.adjusted_quantity, status="created", execution_state="created")
                db.add(order)
                try:
                    db.flush()
    ...
    async def _handle_binance_error(self, setup_id, exc, client):
        with self.session_factory() as db:
            order = db.scalar(select(ExecutionOrder).where(ExecutionOrder.trade_setup_id == setup_id))
            setup = db.get(TradeSetup, setup_id)
            symbol = db.get(Symbol, setup.symbol_id)
            if not order:
                self._event(db, "execution_failed", setup, symbol, reason=str(exc), severity="ERROR")
                db.commit()
            ...
            if exc.unknown:
                ...
                try:
                    ...
                    self._event(db, "exchange_submission_acknowledged", setup, symbol, order=order, metadata={"reconciled": True})
                    db.commit()
                    return {"started": True, "order_id": order.id, "status": order.status}
                except BinanceError:
                    self._event(db, "execution_failed", setup, symbol, order=order, reason="execution_state_unknown", severity="ERROR")
                    db.commit()
                    return {"started": True, "order_id": order.id, "reason": "execution_state_unknown"}
            order.status = "exchange_rejected"
            order.execution_state = "rejected"
            order.rejection_reason = str(exc)
            self._event(db, "exchange_submission_rejected", setup, symbol, order=order, reason=str(exc), severity="WARNING")
            db.commit()
