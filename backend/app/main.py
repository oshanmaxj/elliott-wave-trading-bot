...
async def lifespan(app: FastAPI):
    ...
    try:
        with SessionLocal.begin() as db:
            for symbol in config.default_symbols: ensure_symbol(db, symbol)
        if config.enable_startup_sync:
            tasks.append(asyncio.create_task(HistoricalSyncService().sync_configured(), name="historical-sync"))
...
app = FastAPI(title="Elliott Wave AI Trading Bot", version="1.0.0", description="Phase 1 public-market analysis and paper-trading foundation. No live order execution.", lifespan=lifespan)
...
async def request_logging(request: Request, call_next):
    ...
    log_event("INFO", "api", "request", f"{request.method} {request.url.path}", {"status": response.status_code, "duration_ms": round((time.perf_counter() - started) * 1000, 2)})
    return response
